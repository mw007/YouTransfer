...
LocalFileStorage.prototype.list = function(res, bundlelist) {
    var basedir = this.localstoragepath;

    // console.log("in LFS.list");
    ...
    //                     console.log("pushing filename %s", file.name);
	...
	// 	console.log("calling next() function from localstorage.list");
    //     next(err);
    // });

	var files = fs.readdirSync(basedir);

	// console.log("list of files: %s", JSON.stringify(files));

	for (var i = 0, len = files.length; i < len; i++) {
		var file = files[i];

		// console.log("working file %s", file);

		if (file.match(/.json$/)) {
			var data = fs.readFileSync(path.join(basedir, file), 'utf8');
			var context = JSON.parse(data);

			if (context.bundle === undefined) {
				bundlelist.push(context);
			}
		}
	}
};
